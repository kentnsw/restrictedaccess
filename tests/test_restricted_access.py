...
def test_oneNoncompliantSg(ec2_client, ec2_resource, faker):
    ...
    result = restricted_access.evaluateSecurityGroups()
    assert result[0].startswith(f'INFO: Revoked unrestricted access from Security Group {sg.group_name}({gid})')
    sg.reload()
...
def test_oneNoncompliantSgWithMixedRanges(ec2_client, ec2_resource, faker):
    ...
    authSgIngress(gid, 'tcp', 20, 25, [{'CidrIp': '0.0.0.0/0'}], ec2_client)
    authSgIngress(gid, 'tcp', 3306, 3306, [{'CidrIp': '0.0.0.0/0'}], ec2_client)
    result = restricted_access.evaluateSecurityGroups()
    assert len(result) == 2
    assert result[0].startswith(f'INFO: Revoked unrestricted access from Security Group {sg.group_name}({gid})')
    assert result[0].index("\'FromPort\': 20") > 0
    assert result[1].startswith(f'INFO: Revoked unrestricted access from Security Group {sg.group_name}({gid})')
...
def test_multipleNoncompliantSgs(ec2_client, ec2_resource, faker):
    ...
    authSgIngress(gid2, 'tcp', 3306, 3306, [{'CidrIp': '0.0.0.0/0'}], ec2_client)
    authSgIngress(gid3, 'tcp', 3307, 3307, [{'CidrIp': '200.0.0.0/0'}], ec2_client)
    ...
    result = restricted_access.evaluateSecurityGroups()
    assert len(result) == 2
    assert result[0].startswith(f'INFO: Revoked unrestricted access from Security Group {sg1.group_name}({gid1})')
    assert result[1].startswith(f'INFO: Revoked unrestricted access from Security Group {sg2.group_name}({gid2})')
    sg1.reload()
    sg2.reload()
